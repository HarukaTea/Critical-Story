...
--[[
	Set the state of Lock with the given boolean
]]
...
--[[
	The action of enable shiftlock thread, enable it again to disable it
]]
...
return function(plr: Player)
	...
	Fusion.New("ScreenGui")({
		...
		[Fusion.Children] = {
			Components.Frame({
				...
				ZIndex = 999,
				Position = fromScale(0.985, 0.545),
					...
					Components.HoverImageButton({
						...
						[Fusion.OnEvent("MouseButton1Click")] = function()
							if self.char:FindFirstChild("Humanoid") and self.char.PrimaryPart then
								local state = if self.enabled then "Off" else "On"
	...
	self.char:WaitForChild("Humanoid").Died:Once(clearGarbage)
end
